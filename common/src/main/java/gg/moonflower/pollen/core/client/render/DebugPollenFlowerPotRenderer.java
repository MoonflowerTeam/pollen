...
//        RANDOM.setSeed(pos.asLong());
        List<BlockState> possibleStates = Registry.BLOCK.getRandom(RANDOM_SOURCE).map(Holder::value).orElse(Blocks.AIR).getStateDefinition().getPossibleStates();
        BlockState state = possibleStates.get(RANDOM_SOURCE.nextInt(possibleStates.size()));
