...
public class ParticleExpireNotInBlocksComponent implements CustomParticleComponent, CustomParticleTickComponent {
    ...
    public ParticleExpireNotInBlocksComponent(JsonElement json) throws JsonParseException {
        JsonArray jsonObject = json.getAsJsonArray();
        this.blocks = new Block[jsonObject.size()];
        try {
            for (int i = 0; i < jsonObject.size(); i++) {
                this.blocks[i] = Registry.BLOCK.get(new ResourceLocation(GsonHelper.convertToString(jsonObject.get(i), "minecraft:particle_expire_if_not_in_blocks[" + i + "]")));
            }
