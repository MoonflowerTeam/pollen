...
import gg.moonflower.pollen.mixin.EntityAccessor;
...
public interface PollenFluidBehavior {
    ...
    default void doSplashEffect(Entity entity, Random random) {
        ...
        if(entity instanceof EntityAccessor accessor) {
            if (g < 0.25) {
                entity.playSound(accessor.invokeGetSwimSplashSound(), g, 1.0F + (random.nextFloat() - random.nextFloat()) * 0.4F);
            } else {
                entity.playSound(accessor.invokeGetSwimHighSpeedSplashSound(), g, 1.0F + (random.nextFloat() - random.nextFloat()) * 0.4F);
            }
        }
