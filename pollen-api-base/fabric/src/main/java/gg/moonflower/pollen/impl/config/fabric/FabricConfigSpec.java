...
public class FabricConfigSpec extends UnmodifiableConfigWrapper<UnmodifiableConfig> {
    ...
    public void setConfig(CommentedConfig config) {
        ...
        if (config != null && !this.isCorrect(config)) {
            ...
            this.correct(
                    config,
                    (action, path, incorrectValue, correctedValue) -> LOGGER.warn(CORE, "Incorrect key {} was corrected from {} to its default, {}. {}", DOT_JOINER.join(path), incorrectValue, correctedValue, incorrectValue == correctedValue ? "This seems to be an error." : ""),
                    (action, path, incorrectValue, correctedValue) -> LOGGER.debug(CORE, "The comment on key {} does not match the spec. This may create a backup.", DOT_JOINER.join(path))
            );
    ...
    private void resetCaches(Iterable<Object> configValues) {
        configValues.forEach(value -> {
            if (value instanceof PollinatedConfigBuilder.ConfigValue<?> configValue) {
