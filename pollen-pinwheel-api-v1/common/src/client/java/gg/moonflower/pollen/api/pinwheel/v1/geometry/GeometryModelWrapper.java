...
public class GeometryModelWrapper implements GeometryModel, AnimatedModel {
    ...
    public GeometryModelWrapper(GeometryModel parent) {
        this.parent = parent;
    }
    ...
    public void applyAnimations(float animationTime, MolangRuntime.Builder runtime, float[] weights, AnimationData... animations) {
        if (this.parent instanceof AnimatedModel animatedModel)
            animatedModel.applyAnimations(animationTime, runtime, weights, animations);
    }

    @Override
    public GeometryModelData.Locator[] getLocators(String part) {
        return this.parent instanceof AnimatedModel animatedModel ? animatedModel.getLocators(part) : new GeometryModelData.Locator[0];
    }
