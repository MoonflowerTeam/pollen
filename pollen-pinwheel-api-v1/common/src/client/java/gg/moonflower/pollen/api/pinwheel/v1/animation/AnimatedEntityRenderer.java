...
public abstract class AnimatedEntityRenderer<T extends Mob & AnimatedEntity> extends GeometryEntityRenderer<T> {
    ...
    public ResourceLocation[] getAnimations(T entity) {
        ResourceLocation[] animations = entity.isNoAnimationPlaying() ? entity.getIdleAnimationState().animations() : entity.getAnimationState().animations();
        if (entity.isAnimationTransitioning())
            return Streams.concat(Arrays.stream(animations), Arrays.stream(entity.getTransitionAnimationState().animations())).toArray(ResourceLocation[]::new);
        return animations;
    }
    ...
    public float[] getAnimationWeights(T entity, float partialTicks) {
        if (entity.isAnimationTransitioning()) {
            ...
            int transitionAnimationStart = animations.length - entity.getTransitionAnimationState().animations().length;
