...
/**
 * <p>Defines an entity as having animations states for animating animated models.</p>
...
public interface AnimatedEntity extends AnimationEffectSource {
    ...
    default void animationTick() {
        ...
        } else if (animationTick >= animationState.tickDuration() - 1) { // only stop animation if not transitioning
    ...
    default void handleSoundEffect(AnimationData animation, AnimationData.SoundEffect soundEffect) {
        ...
        ResourceLocation sound = ResourceLocation.tryParse(soundEffect.effect());
        if (sound != null) {
            MolangRuntime runtime = MolangRuntime.runtime().create(1.0F); // 1.0 is the default pitch and volume
            Minecraft.getInstance().getSoundManager().play(new AnimationEffectSound(sound, entity.getSoundSource(), animation, entity, soundEffect.pitch().safeResolve(runtime), soundEffect.volume().safeResolve(runtime), soundEffect.loop()));
        }
    }
